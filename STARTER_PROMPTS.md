# Starter Prompts — venture-payroll-agent
...
```
I have the project scaffold for venture-payroll-agent. Check what's already been set up (git log, git remote -v, node_modules, .env.local) and pick up from where things left off. The full setup list is:
1. npm install
...
```
Let's get this deployed to Cloud Run. First check if any of this is already done (check .auto-memory/reference_venture-payroll-agent.md for existing values). The full list is:
1. Create a GCP project: gcloud projects create venture-payroll-agent
...
5. Deploy to Cloud Run: gcloud run deploy venture-payroll-agent --source . --region us-east1 --project venture-payroll-agent --allow-unauthenticated
6. Verify it's live at the Cloud Run URL
7. Write the Cloud Run URL and GCP project ID to .auto-memory/reference_venture-payroll-agent.md
...
## Session 4: CoAdvantage Integration

### 4a — Mock layer
```
Build a CoAdvantage integration module. Start with a mock data layer that mirrors the exact shape of the real CoAdvantage API response. Wire it into the relevant views. Mock must work before we touch real credentials.
```

### 4b — Live integration
```
I've added my CoAdvantage credentials to .env.local. Now:
1. Replace the mock with real CoAdvantage API calls using the env vars
...
## Session 5: Five9 Integration

### 5a — Mock layer
```
Build a Five9 integration module. Start with a mock data layer that mirrors the exact shape of the real Five9 API response. Wire it into the relevant views. Mock must work before we touch real credentials.
```

### 5b — Live integration
```
I've added my Five9 credentials to .env.local. Now:
1. Replace the mock with real Five9 API calls using the env vars
2. Handle auth and error cases (token expiry, rate limits, unexpected responses)
3. Confirm all field names and write them to memory as [Tier 1]
4. Test with live data end-to-end
5. Commit and write a memory file
...
## Session 6: Google Chat Integration

### 6a — Mock layer
```
Build a Google Chat integration module. Start with a mock data layer that mirrors the exact shape of the real Google Chat API response. Wire it into the relevant views. Mock must work before we touch real credentials.
```

### 6b — Live integration
```
I've added my Google Chat credentials to .env.local. Now:
1. Replace the mock with real Google Chat API calls using the env vars
2. Handle auth and error cases (token expiry, rate limits, unexpected responses)
3. Confirm all field names and write them to memory as [Tier 1]
4. Test with live data end-to-end
5. Commit and write a memory file
...
## Session 7: Google Workspace Integration

### 7a — Mock layer
```
Build a Google Workspace integration module. Start with a mock data layer that mirrors the exact shape of the real Google Workspace API response. Wire it into the relevant views. Mock must work before we touch real credentials.
```

### 7b — Live integration
```
I've added my Google Workspace credentials to .env.local. Now:
1. Replace the mock with real Google Workspace API calls using the env vars
2. Handle auth and error cases (token expiry, rate limits, unexpected responses)
3. Confirm all field names and write them to memory as [Tier 1]
4. Test with live data end-to-end
5. Commit and write a memory file
...
## Session 8: Forge Integration

### 8a — Mock layer
```
Build a Forge integration module. Start with a mock data layer that mirrors the exact shape of the real Forge API response. Wire it into the relevant views. Mock must work before we touch real credentials.
```

### 8b — Live integration
```
I've added my Forge credentials to .env.local. Now:
1. Replace the mock with real Forge API calls using the env vars
2. Handle auth and error cases (token expiry, rate limits, unexpected responses)
3. Confirm all field names and write them to memory as [Tier 1]
4. Test with live data end-to-end
5. Commit and write a memory file
...
### Deploy
```
Deploy the current state to Cloud Run (service: venture-payroll-agent, project: venture-payroll-agent). First verify I have gcloud and docker installed (check with which). Walk me through each step from ~/Downloads/venture-payroll-agent. Make sure env vars are set (use --update-env-vars, never --set-env-vars) and the service is healthy after deploy. Update .auto-memory/ with the live URL if this is a first deploy.
```
