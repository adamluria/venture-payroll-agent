# venture-payroll-agent — AI Assistant Instructions
...
## Project context:
- **Description**: AI-powered payroll verification and commission calculation agent for Venture Home Solar's inside sales team that automates timesheet validation and commission calculations.
...
## Quick commands (run from repo root: `~/Downloads/venture-payroll-agent`):
```bash
npm run dev                                       # local dev server
docker build -t venture-payroll-agent .                        # test Docker build locally
